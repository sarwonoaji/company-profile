...
<div class="flex items-center justify-between mb-4">
    ...
    <a href="{{ route('admin.banners.index') }}" class="text-sm text-blue-600 hover:underline">Kembali</a>
</div>

<form method="POST" action="{{ route('admin.banners.store') }}" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
@csrf

    <label class="block mb-2">Judul</label>
    <input name="judul" value="{{ old('judul') }}" class="w-full border rounded px-3 py-2 mb-2 focus:outline-none focus:ring-2 focus:ring-blue-200">
    @error('judul') <p class="text-red-600 text-sm mb-2">{{ $message }}</p> @enderror

    <label class="block mb-2">Deskripsi</label>
    <textarea name="deskripsi" id="editor" class="w-full border rounded px-3 py-2 mb-2">{{ old('deskripsi') }}</textarea>
